...
import { createNewAccount, createNewHashedAccount } from '../factory/accounts.js';
...
beforeAll(async () => {
  ...
  validToken = goToken(newAccount._id.toHexString());
});
...
describe('POST /api/admin/accounts', () => {
  ...
  ])('deve retornar status 422, quando um usuário fornecer um body inválido. (%s)', async (_, mock) => {
...
describe('POST /api/accounts/login', () => {
  ...
  ])('deve retornar status 404, quando um usuário fornecer %s.', async (title, mock) => {
